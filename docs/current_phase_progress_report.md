...
需要特别说明：Tomorrowland 2025 官方 lineup/timetable 已经从 staging 表提升到核心业务表，并且已新增 `performance_artists` 多对多表来表达多人合作演出。Defqon.1 和 Awakenings 目前还没有完成官方业务数据抓取入库。
...
| `artists` | 731 |
| `performances` | 813 |
| `performance_artists` | 871 |
| `ticket_types` | 8 |

Tomorrowland 2025 在正式表中的日期已经更新为官方 timetable 覆盖范围：
...
随后 `sql/13_add_performance_artists_model.sql` 将演出模型从“每条 performance 绑定一个 artist_id”调整为：

- `performances`：表示一个官方节目块或演出时段
- `performance_artists`：表示节目块与艺人的多对多关系，并用 `artist_order` 保留官方艺人顺序

由于 staging 中有 3 条 MC/host 标注的开始时间等于结束时间，正式表只保留 806 条满足 `end_time > start_time` 的 Tomorrowland performance block。按艺人展开后，`v_tomorrowland_2025_timetable` 和导出文件中仍有 864 行。
...
|---|---|---|---|---|---|
| Tomorrowland 2025 | 已有 | 已入正式表 | 未完成 | 近似值 | 官方业务数据样板已完成 |
...
10. 在确认质量后提升到正式业务表
11. 对合作演出使用 `performance_artists` 多对多表保留官方节目块和艺人关联
...
1. 使用 `v_site_score_explanation` 制作报告表和 QGIS 地图
   - 目标：解释每个候选地得分来源，包括机场、人口、生态和噪声敏感设施指标
   - 已导出 `exports/site_score_explanation.csv`

2. 整理最终报告中的空间分析结果
   - 使用 `exports/dynamic_site_scores_top20.csv`
   - 使用 `exports/affected_population_5km.csv`
   - 使用 `exports/real_venue_match_validation.csv`

3. 为 Defqon.1 2025 寻找官方页面或 Wayback 快照
   - 优先 lineup / timetable / tickets 页面
   - 找到 URL 后用浏览器抓包脚本保存 raw 数据

4. 为 Awakenings Summer Festival 2025 寻找官方页面或 Wayback 快照
   - 优先 lineup / timetable / tickets 页面
   - 找到 URL 后重复 Tomorrowland 的 staging 流程

5. 补 Tomorrowland 2025 票价数据
...
7. 为 Defqon.1 和 Awakenings 建立对应 staging 和 promote 脚本
   - 解析 JSON 或页面结构
   - 导入 staging
   - 审核后提升正式表
...
3. 继续完善合作演出建模
   - 当前已新增 `performance_artists` 多对多表
   - 后续可增加 `role` 枚举约束或官方 artist ID 字段
   - 可进一步区分 DJ、host、MC、b2b 等角色
